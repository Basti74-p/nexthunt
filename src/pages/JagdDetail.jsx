...
export default function JagdDetail() {
  ...
  const { tenant, isTenantOwner, isPlatformAdmin, user } = useAuth();
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-4 mt-3 text-sm text-gray-400">
                <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{dateStr}{jagd.uhrzeit_start ? ` · ${jagd.uhrzeit_start}` : ""}{jagd.uhrzeit_ende ? ` – ${jagd.uhrzeit_ende}` : ""}</span>
                {jagd.treffpunkt && <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />{jagd.treffpunkt}</span>}
              </div>
            </div>
            {canManage && (
      ...
      {editOpen && (
        <JagdDialog open={editOpen} onClose={() => { setEditOpen(false); queryClient.invalidateQueries({ queryKey: ["jagd", jagdId] }); }} jagd={jagd} />
      )}
    </div>
  );
}
