...
export default function RevierAufgaben({ revier }) {
  ...
  const { data: aufgaben = [] } = useQuery({
    queryKey: ["aufgaben", revier.id],
    queryFn: () => base44.entities.Aufgabe.filter({ revier_id: revier.id }),
  });

  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.Aufgabe.create({ ...data, revier_id: revier.id, tenant_id: tenant.id, status: "offen" }),
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      {aufgaben.length === 0 ? (
        <EmptyState icon={ListTodo} title="Keine Aufgaben" description="Erstellen Sie Aufgaben für Ihr Team." />
      ) : (
        <div className="space-y-4">
          {open.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-gray-500 px-1">Offen ({open.length})</h3>
              {open.map(a => (
                <div key={a.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                  <button
                    ...
                    className="w-6 h-6 rounded-full border-2 border-gray-300 hover:border-[#0F2F23] flex items-center justify-center transition-colors shrink-0"
                  />
          ...
          {done.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-gray-400 px-1">Erledigt ({done.length})</h3>
              {done.map(a => (
                <div key={a.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3 opacity-50">
                  ...
                  <p className="font-medium text-gray-500 line-through flex-1">{a.title}</p>
                </div>
              ))}
            </div>
          )}
