...
function ProtokollForm({ formData, set, onClose, onSubmit, isEdit, isPending, uploadingPhoto, fileInputRef, handlePhotoUpload, removePhoto, isMobile }) {
  return (
    <form onSubmit={onSubmit} className={isMobile ? "flex flex-col overflow-hidden" : "space-y-0"}>
      <div className={isMobile
        ? "overflow-y-auto px-4 space-y-5 pb-2 max-w-sm mx-auto w-full"
        : "overflow-y-auto max-h-[70vh] px-1 space-y-5 pr-2"
      }>
                ...
                <div>
                  ...
                  <Select value={formData.schadensart} onValueChange={(v) => set("schadensart", v)}>
                    ...
                    <SelectContent className="bg-[#2d2d2d] border-[#3a3a3a]">
                      {SCHADENSART_OPTIONS.map(o => <SelectItem key={o.value} value={o.value} className="text-gray-100">{o.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Select value={formData.schwere} onValueChange={(v) => set("schwere", v)}>
                    <SelectTrigger className="bg-[#1a1a1a] border-[#3a3a3a] text-gray-100"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-[#2d2d2d] border-[#3a3a3a]">
                      {SCHWERE_OPTIONS.map(o => <SelectItem key={o.value} value={o.value} className="text-gray-100">{o.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div>
        ...
        <section>
          ...
          <div className="space-y-2">
            <Select value={formData.status} onValueChange={(v) => set("status", v)}>
              <SelectTrigger className="bg-[#1a1a1a] border-[#3a3a3a] text-gray-100"><SelectValue /></SelectTrigger>
              <SelectContent className="bg-[#2d2d2d] border-[#3a3a3a]">
                {STATUS_OPTIONS.map(o => <SelectItem key={o.value} value={o.value} className="text-gray-100">{o.label}</SelectItem>)}
              </SelectContent>
            </Select>
      ...
      {/* Buttons */}
      <div className={isMobile
        ? "px-4 pt-2 border-t border-[#3a3a3a] flex gap-2 max-w-sm mx-auto w-full"
...
export default function SchadensprotokollDialog({ isOpen, onClose, schaden, einrichtung, tenantId }) {
    ...
    onMutate: (data) => {
      if (isEdit) {
        ...
        queryClient.setQueryData(["schadensprotokoll", einrichtung?.id], old => [
          ...(old || []),
          { ...data, id: "temp-" + Date.now(), tenant_id: tenantId, revier_id: einrichtung.revier_id, einrichtung_id: einrichtung.id, einrichtung_name: einrichtung.name }
        ]);
      }
    },
    onSuccess: () => {
  ...
  const formProps = { formData, set, onClose, onSubmit: handleSubmit, isEdit, isPending: mutation.isPending, uploadingPhoto, fileInputRef, handlePhotoUpload, removePhoto };
    ...
    return (
      <Drawer open={isOpen} onOpenChange={onClose}>
        <DrawerContent className="bg-[#2d2d2d] border-t border-[#3a3a3a] max-h-[92dvh]">
          ...
          <ProtokollForm {...formProps} isMobile={true} />
        </DrawerContent>
      </Drawer>
    );
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl bg-[#2d2d2d] border-[#3a3a3a] text-white">
        <DialogHeader>
        ...
        <ProtokollForm {...formProps} isMobile={false} />
      </DialogContent>
    </Dialog>
  );
}