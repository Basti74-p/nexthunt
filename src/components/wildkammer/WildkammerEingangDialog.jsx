...
function EingangForm({ form, set, onClose, onSave, reviere, strecken, handleStreckeLink, canSave, isMobile }) {
  return (
    <div className={isMobile ? "overflow-y-auto px-4 space-y-4 pb-2" : "space-y-4 max-h-[70vh] overflow-y-auto pr-1"}>
...
export default function WildkammerEingangDialog({ open, onClose, onSave, reviere, strecken = [], wildmarken = [] }) {
  ...
  const formProps = { form, set, onClose, onSave, reviere, strecken, handleStreckeLink, canSave };
    ...
    return (
      <Drawer open={open} onOpenChange={v => !v && onClose()}>
        <DrawerContent className="bg-[#2d2d2d] border-t border-[#3a3a3a] max-h-[92dvh]">
          ...
          <EingangForm {...formProps} isMobile={true} />
        </DrawerContent>
      </Drawer>
    );
  }
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-2xl bg-[#2d2d2d] border-[#3a3a3a] text-white">
        ...
        <EingangForm {...formProps} isMobile={false} />
      </DialogContent>
    </Dialog>
  );
}