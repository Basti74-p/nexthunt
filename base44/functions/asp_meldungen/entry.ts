...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.entities.ASPMeldung.filter(filter);
    ...
    // CREATE
    if (action === "create") {
      if (!body.datum_fund) return json({ error: "datum_fund required" }, 400);
      if (!body.fund_typ) return json({ error: "fund_typ required" }, 400);
      const created = await base44.entities.ASPMeldung.create({
        tenant_id,
        revier_id: revier_id || "",
        wildart: "Wildschwein",
        anzahl_stueck: 1,
    ...
    if (action === "update") {
      ...
      const updated = await base44.entities.ASPMeldung.update(id, rest);
      return json(updated);
    }
