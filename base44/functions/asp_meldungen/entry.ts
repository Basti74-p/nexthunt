...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.asServiceRole.entities.ASPMeldung.filter(filter);
    ...
    // CREATE
    if (action === 'create') {
      if (!rest.datum_fund) return Response.json({ error: 'datum_fund required' }, { status: 400, headers: corsHeaders });
      if (!rest.fund_typ) return Response.json({ error: 'fund_typ required' }, { status: 400, headers: corsHeaders });
      const data = await base44.asServiceRole.entities.ASPMeldung.create({
        wildart: 'Wildschwein', anzahl_stueck: 1, status: 'offen', ergebnis: 'ausstehend',
        ...rest, tenant_id, revier_id: revier_id || '',
      });
    ...
    if (action === 'update') {
      ...
      const data = await base44.asServiceRole.entities.ASPMeldung.update(id, rest);
      return Response.json({ data, sync_timestamp: new Date().toISOString() }, { headers: corsHeaders });
    }
