...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.entities.SchwarzwildSichtung.filter(filter);
    ...
    if (action === "create") {
      ...
      const created = await base44.entities.SchwarzwildSichtung.create({
        tenant_id,
        revier_id: revier_id || "",
        source: "mobile",
        ...rest,
    ...
    if (action === "update") {
      ...
      const updated = await base44.entities.SchwarzwildSichtung.update(id, rest);
      return json(updated);
    }
    ...
    if (action === "delete") {
      if (!id) return json({ error: "id required" }, 400);
      await base44.entities.SchwarzwildSichtung.delete(id);
