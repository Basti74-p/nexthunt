...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.asServiceRole.entities.Kirrung.filter(filter);
    ...
    if (action === 'create') {
      ...
      const data = await base44.asServiceRole.entities.Kirrung.create({
        aktiv: true, ...rest, tenant_id, revier_id: revier_id || '',
      });
    ...
    if (action === 'update') {
      ...
      const data = await base44.asServiceRole.entities.Kirrung.update(id, rest);
      return Response.json({ data, sync_timestamp: new Date().toISOString() }, { headers: corsHeaders });
    }
