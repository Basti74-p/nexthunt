...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.entities.Kirrung.filter(filter);
    ...
    if (action === "create") {
      ...
      const created = await base44.entities.Kirrung.create({
        tenant_id,
        revier_id: revier_id || "",
        aktiv: true,
        ...rest,
    ...
    if (action === "update") {
      ...
      const updated = await base44.entities.Kirrung.update(id, rest);
      return json(updated);
    }
