...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.action === 'create') {
      ...
      const created = await base44.asServiceRole.entities.WolfSighting.create({ ...body, tenant_id: payload.tenant_id });
    ...
    const data = await base44.asServiceRole.entities.WolfSighting.filter(filter);
