...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.entities.SchwarzwildSchaden.filter(filter);
    ...
    // CREATE
    if (action === "create") {
      if (!body.datum) return json({ error: "datum required" }, 400);
      if (!body.schadensart) return json({ error: "schadensart required" }, 400);
      const created = await base44.entities.SchwarzwildSchaden.create({
        tenant_id,
        revier_id: revier_id || "",
        status: "offen",
        ...rest,
    ...
    if (action === "update") {
      ...
      const updated = await base44.entities.SchwarzwildSchaden.update(id, rest);
      return json(updated);
    }
