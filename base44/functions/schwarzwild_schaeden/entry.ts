...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.asServiceRole.entities.SchwarzwildSchaden.filter(filter);
    ...
    if (action === 'create') {
      ...
      if (!rest.schadensart) return Response.json({ error: 'schadensart required' }, { status: 400, headers: corsHeaders });
      const data = await base44.asServiceRole.entities.SchwarzwildSchaden.create({
        status: 'offen', ...rest, tenant_id, revier_id: revier_id || '',
      });
    ...
    if (action === 'update') {
      ...
      const data = await base44.asServiceRole.entities.SchwarzwildSchaden.update(id, rest);
      return Response.json({ data, sync_timestamp: new Date().toISOString() }, { headers: corsHeaders });
    }
