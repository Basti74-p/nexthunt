...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action) {
      ...
      let data = await base44.asServiceRole.entities.SchwarzwildRotte.filter(filter);
    ...
    // CREATE
    if (action === 'create') {
      if (!rest.name) return Response.json({ error: 'name required' }, { status: 400, headers: corsHeaders });
      const data = await base44.asServiceRole.entities.SchwarzwildRotte.create({
    ...
    if (action === 'update') {
      ...
      const data = await base44.asServiceRole.entities.SchwarzwildRotte.update(id, rest);
      return Response.json({ data, sync_timestamp: new Date().toISOString() }, { headers: corsHeaders });
    }
